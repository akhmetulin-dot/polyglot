...
Vocabulary trainer with session-based spaced repetition. Words carry
translations (Polish / German / English), a mnemonic, priority and grouping
metadata; the trainer schedules reviews by session number rather than by
wall-clock date, so a break from studying never buries you in overdue cards.

This is a self-contained pnpm workspace. It shares nothing with the mind map
project — separate database, separate dependencies, separate Netlify site.

## Layout

```
apps/web/                  React 19 + Vite 7 + Tailwind 4 single-page app
...
`http://localhost:5000`. Run the API next to it in a second terminal:
...
```bash
netlify dev --filter @polyglot/web
```
...
enabled automatically whenever the URL carries `sslmode=require`,
`verify-ca` or `verify-full`; override with `PGSSL=0` or `PGSSL=1` if you need
to. `NETLIFY_DATABASE_URL` is read as a fallback, so Netlify DB works with no
...
| `pnpm --filter @polyglot/db studio` | open Drizzle Studio                          |

Use `db:push` for throwaway local databases and `db:generate` + `db:migrate`
for anything you care about. Tables: `words`, `settings`, `word_events`,
`custom_tags`.
...
- **Base directory** — `polyglot` (leave empty once this project has its own
  repository)
